...
export default function PlayerEditor({ onNodeSelect }) {
  ...
  const processLoadedJson = useCallback(
    (content, meta = {}) => {
      const { fileHandle = null, fileName = '', skipConfirm = false } = meta;
      try {
  ...
  // 检查URL参数和localStorage，自动加载新文件（只运行一次）
  ...
  React.useEffect(() => {
    ...
    // 检查是否有新文件需要加载
    if (action === 'new' && filePath) {
      const fileKey = `newChapterFile_${filePath}`;
      const jsonContent = localStorage.getItem(fileKey);
      
      if (jsonContent) {
        hasAutoLoadedRef.current = true;
        try {
          const content = JSON.parse(jsonContent);
          // 使用processLoadedJson加载文件，但不显示确认对话框
          const success = processLoadedJson(content, {
            fileHandle: null,
            fileName: localStorage.getItem('newChapterFileName') || `${filePath}.json`,
          ...
          if (success) {
            ...
            localStorage.removeItem('newChapterFileName');
            localStorage.removeItem('newChapterFilePath');
            localStorage.removeItem('newChapterAction');
